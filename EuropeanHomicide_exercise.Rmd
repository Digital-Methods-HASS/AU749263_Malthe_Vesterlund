...
author: "Malthe Asbjørn Vesterlund"
date: "`r Sys.Date()`"
output: 
  html_document:
    toc: true
...
```{r relabel column}
## New table
Western_Europe2 <- read_csv("data/homicide-rates-across-western-europe.csv")

Western_Europe2 <- Western_Europe2 %>% 
...
```{r eval = FALSE}
#ggplot(data = Western_Europe) + 
## Homicide rate for individual countries

Western_Europe2 %>% 
  ggplot(aes(x=Year,y=homicides_per_100k,color=Country,facet_wrap(Country)))+
  geom_line()+
  facet_wrap(~Country)+
  theme_bw()+
...
Compared to the homicides above, it is clear that the average mean time of Danish
monarchs are rising considerable, contrary to homicides, which are falling.

```{r eval = FALSE}
## Monarchs exercise:
monarchs <- read.csv("data/Uge 10 Lektier - Ark1.csv")

monarchs %>% 
  mutate(duration = End_reign - Start_reign, midyear = End_reign - duration/2) %>% 
...
To conclude that we are more civilised today because of one single reason can more often than not be a futile way of viewing history as well as our modern-day society. When deciding for an answer, researchers would often turn to multiple factors to argue about a society's level of "civilisation". Even the term "civilised" has been a term used to marginalise other groups of people. However, one cannot deny the immense progress that many Western European countries has made regarding the fall of homicide rates. It is a sign that our societies have become significantly safer and more pleasant to live in. Therefore, if one had to choose between deciding if we are more civilised today than before or not, solely based on the data visualisations of homicide rates, it would make far more sense to answer "yes" to the question.
