...
dk <- leaflet() %>%   # assign the base location to an object
  setView(6.2616074,56.1235177, zoom = 5)

esri <- grep("^Esri", providers, value = TRUE)

for (provider in esri) {
  dk <- dk %>% addProviderTiles(provider, group = provider)
}
...
# Your brief answer
#The first map without clustering tells us more accurately where the 
#locations are located, however the second map clusteres the locations in close
#proximity better, making the second map more easy to read.
...
saveWidget(DANmap, "DANmap Week 14 assignment.html", selfcontained = TRUE)
